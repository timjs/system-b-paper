...
We present System B, a calculus distinguishing _borrowed_, _owned_, and _shared_ bindings, enabling multiple optimized implementations from a single function definition. Bindings that are only borrowed, coincide with _second class values_ and can be allocated on the stack. While providing a functional interface, _uniquely_ owned memory at runtime is safely reusable. We demonstrate System B's feasibility by positioning it along efficiency and polymorphism axes relative to existing languages.
